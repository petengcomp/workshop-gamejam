...
export default class Pause extends Phaser.Scene {
  ...
  create() {
    /* adicionando áudio */
    this.gameOverAudio = this.sound.add("pauseSound", {
      loop: false,
      volume: 0.1,
    });
    ...
    const playPauseButton = this.add
      .image(width / 2, height / 3, "resume")
      .setScale(0.5)
      .setInteractive();
    playPauseButton.on("pointerover", () => {
      playPauseButton.setScale(0.22);
    });
    playPauseButton.on("pointerout", () => {
      playPauseButton.setScale(0.5);
    });
    ...
    const restart = this.add
      .image(100, 410, "restart")
      .setScale(0.4)
      .setInteractive();
    restart.on("pointerover", () => {
      restart.setScale(0.22);
    });
  ...
  update() {
    ...
    if (this.keys.shift.isDown) this.resume();
  }
